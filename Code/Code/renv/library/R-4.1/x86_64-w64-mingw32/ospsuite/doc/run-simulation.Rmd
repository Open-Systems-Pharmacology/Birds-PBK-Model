...
In some cases, the user might want to run multiple simulations in parallel. This can be achieved easily by simply passing a list of simulations the `runSimulations` function. However, only individual simulations (i.e., the `population` argument must remain empty) 
are supported.
